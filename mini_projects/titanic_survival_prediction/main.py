...
class TitanicDataset(Dataset):
    ...
    #处理Embarked列：不同地方上船，也可能与社会阶层有关
        #mode()把空值填为众数
        self.train['Embarked']=self.train['Embarked'].fillna(self.train['Embarked'].mode()[0])
    ...
    #提取y
        ...
        if self.is_train:
            self.y_data=torch.tensor(self.train[['Survived']].values,dtype=torch.float32)
...
submission=pd.DataFrame({'PassengerId':dataset.passenger_id.values,'Survived':pred})
